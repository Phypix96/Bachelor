...
for i=1:41
  for j=1:41
    ...
    β=(j-1)/40
    for l=1:L
      Err[i,j]+=abs(density[i,j,l]-density_prof[i,j,l])^2
    end
    mean_flow[i,j]=mean(flow[i,j,:])
    std_flow[i,j]=std(flow[i,j,2:L-1])/mean_flow[i,j]
    mean_density=mean(density[i,j,:])
    Err[i,j]=sqrt(Err[i,j])
    min_entropy[i,j]=minimum(ent_entropy_MPS[i,j,2:L-1])
  end
end
x1=linspace(0,1,40)
...
#contour(x1,x1,ent_entropy_MPS[2:41,2:41],fill=true,nlevels=50,seriescolor=:magma_r)
#contour(x2,x2,Err[5:41,5:41],fill=true,nlevels=50,seriescolor=:magma_r)
...
#contour(x1,x1,imfilter(std_flow[2:41,2:41],Kernel.gaussian(0.5)),seriescolor=:magma_r,xlims=(0,1),ylims=(0,1),fill=true, nlevels=50)
...
#savefig("Data/stoc_entropy_full.png")
